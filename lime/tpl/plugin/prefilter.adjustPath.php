...
//
	...
	$tpl_path = dirname($tpl->tpl_dir).'/';
//	pr(($tpl->tpl_dir));
//	pr(($tpl_path));
//	echo $tpl_path = rtrim($tpl->tpl_path,'/')."/".trim($tpl->skin,'/').'/';//.$path;
...
*/
	...
	preg_match_all($pattern, $source, $m, PREG_SPLIT_DELIM_CAPTURE);
//pr($m);
	$m=array();
...
//		$split[$i]='/'.implode('/',array_slice(explode('/', $src),$base_cnt));
		$base_cnt = count(explode('/', $base_path.rtrim(BASE_PATH,'/')));
		$split[$i]='<?php echo BASE_PATH;?>'.implode('/',array_slice(explode('/', $src),$base_cnt));
